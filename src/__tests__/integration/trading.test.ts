...
describe('Trading Operations Integration Tests', () => {
  ...
  beforeAll(async () => {
    ...
    try {
      ...
      console.log(`Opening maker position with margin: ${marginAmount} USDC, liquidity: ${liquidity.toString()}`);
      console.log(`Tight price range: ${tightPriceLower.toFixed(2)} - ${tightPriceUpper.toFixed(2)}`);
  ...
  describe('openMakerPosition', () => {
    ...
    it('should open a maker (LP) position', async () => {
      ...
      const priceLower = Math.pow(1.0001, alignedTickLower);
      const priceUpper = Math.pow(1.0001, alignedTickUpper);
    ...
    it('should require pre-aligned ticks', async () => {
      ...
      const priceUpper = 5678.90; // Likely not aligned to tick spacing
      ...
      if (tickLower !== alignedTickLower || tickUpper !== alignedTickUpper) {
        ...
        const liquidity = await estimateLiquidity(context, alignedTickLower, alignedTickUpper, marginScaled);
